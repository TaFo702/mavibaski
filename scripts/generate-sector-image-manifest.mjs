...
function getAllPublicImages(dir) {
  let results = [];
  ...
  for (const item of fs.readdirSync(dir)) {
    ...
    if (stat.isDirectory()) {
      results = results.concat(getAllPublicImages(full));
    } else if (item.endsWith('.webp') || item.endsWith('.png') || item.endsWith('.jpg')) {
      results.push(path.relative(path.join(projectRoot, 'public'), full));
    }
  }
  return results;
}

const allScannedImages = getAllPublicImages(path.join(projectRoot, 'public', 'images'));
const combinedBlogImages = Array.from(new Set([...BLOG_IMAGES, ...allScannedImages]));

const blogManifest = {};
for (const relPath of combinedBlogImages) {
...
// Etiket Sayfası Görselleri (6 yuva)
const ETIKET_IMAGES = [
  'images/etiket/yapiskanli-etiket-baski.webp',
  'images/etiket/kuse-gorsel-etiket.webp',
  'images/etiket/seffaf-etiket-baski.webp',
  'images/etiket/rulet-etiket-tasarimi.webp',
  'images/etiket/etiket-baski-fiyatlari.webp',
  'images/etiket/etiket-baski-ornegi.webp'
];

const etiketManifest = {};
for (const relPath of ETIKET_IMAGES) {
  const normWithSlash = relPath.startsWith('/') ? relPath : `/${relPath}`;
  const normWithoutSlash = relPath.replace(/^\//, '');
  const fileName = path.basename(relPath);
  const fullPath = path.join(projectRoot, 'public', normWithoutSlash);
  ...
  etiketManifest[normWithSlash] = isValid;
  etiketManifest[normWithoutSlash] = isValid;
  etiketManifest[fileName] = isValid;
}
...
export const ETIKET_IMAGE_MANIFEST = ${JSON.stringify(etiketManifest, null, 2)} as const;
