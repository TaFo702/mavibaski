import { AMBALAJ_IMAGE_MANIFEST } from '../generated/sectorImageManifest';
...
export interface AmbalajPriceRow {
  code: string;
  name: string;
  desc: string;
  miktar: string;
...
export const AMBALAJ_PRICE_TABLE: AmbalajPriceRow[] = [
  {
    code: "AMB1",
    name: "Ambalaj Kağıdı Baskı - 70 gr. Sülfit Kağıt",
    desc: "Ambalaj Kağıdı Baskı - 70 gr. Sülfit Kağıt",
    miktar: "100 kg",
    price: "Fiyat Alınız"
  },
  {
    code: "AMB2",
    name: "Ambalaj Kağıdı Baskı - 135 gr Kuşe - 70 x 100 cm",
    desc: "Ambalaj Kağıdı Baskı - 135 gr Kuşe - 70 x 100 cm",
    miktar: "1.000 Adet",
    price: "Fiyat Alınız"
  },
  {
    code: "AMB3",
    name: "35 x 50 cm - 30 gr - Pelür Kağıdı",
    desc: "35 x 50 cm - 30 gr - Pelür Kağıdı",
    miktar: "1.000 Adet",
    price: "Fiyat Alınız"
  }
];
...
export const AMBALAJ_SEO_METADATA = {
  ...
  ogImage: "https://mavibasim.com/images/ambalaj/baskili-ambalaj-kagidi.webp"
};
...
export const AMBALAJ_PRODUCTS: AmbalajProductItem[] = [
  ...
  },
  {
    ...
    imgAlt: "135 gr kuşe baskılı ambalaj kâğıdı, hediye paketleme ruloları ve kurumsal sarım örneği",
    desc: "Pürüzsüz kuşe yüzeyi ve beyazlık derecesiyle ofset baskı seçenekleri sunar. Büyük ebatlı hediye paketleme ve görsel ağırlıklı kurumsal sunumlar için değerlendirilebilir. Sarım esnekliğinin korunması gereken projelerde, selefon gibi ilave yüzey uygulamaları kullanılmadan ofset baskı seçeneği değerlendirilebilir.",
    features: [
  ...
  },
  {
    ...
    price: "Fiyat Alınız",
    image: "",
    imgAlt: "30 gr pelür kâğıdıyla tekstil ürünü ve kutu içi sarım uygulaması",
...
export const AMBALAJ_CONSIDERATIONS = [
  ...
  },
  {
    ...
    desc: "135 gr kuşe kâğıt hediye paketi sarımlarında görsel baskı seçenekleri sunar; kâğıda selefon uygulanırsa sertleşerek sarım esnekliğini kaybedebilir. Bu nedenle sarım esnekliğinin önemli olduğu projelerde, selefonsuz baskı seçeneği değerlendirilebilir. Pelür kâğıdı ise hafif ve esnek yapısıyla kutu içi sarım seçeneği sunabilir."
  },
  {
    id: "dikkat-kesim",
...
export const AMBALAJ_SCHEMAS = {
  ...
  serviceSchema: {
    "@context": "https://schema.org",
    "@type": "Service",
    "@id": "https://mavibasim.com/ambalaj#service",
...
export function generateAmbalajSSRBodyContent(): string {
  ...
  const productsHtml = AMBALAJ_PRODUCTS.map(p => {
    const isAvail = Boolean(p.image && (AMBALAJ_IMAGE_MANIFEST as Record<string, boolean>)[p.image]);
    const mediaHtml = isAvail
      ? `\n      <figure class="my-4"><img src="${p.image}" alt="${p.imgAlt}" width="600" height="400" loading="lazy" class="w-full h-auto rounded-xl" /><figcaption class="text-xs text-gray-500 mt-1">${p.name}</figcaption></figure>`
      : `\n      <div class="ambalaj-product-placeholder my-4" style="aspect-ratio:3/2;background:#f8fafc;border:1px dashed #cbd5e1;border-radius:0.75rem;display:flex;align-items:center;justify-content:center;text-align:center;padding:1rem;"><span style="font-size:11px;color:#64748b;">${p.name} (Görsel Hazırlanıyor)</span></div>`;
    const featureItems = p.features.map(f => `        <li>${f}</li>`).join("\n");
    return `    <article id="${p.code.toLowerCase()}">
      <h3>${p.name}</h3>
      <p>${p.desc}</p>${mediaHtml}
      <ul>
${featureItems}
      </ul>
  ...
  const priceTableRowsHtml = AMBALAJ_PRICE_TABLE.map(item => {
    return `          <tr>
            <th class="p-2 border font-bold text-gray-900" scope="row">${item.code}</th>
            <td class="p-2 border">${item.name}</td>
            <td class="p-2 border text-center">${item.miktar}</td>
            <td class="p-2 border text-center font-bold">${item.price}</td>
            <td class="p-2 border text-center">
              <a href="https://wa.me/905366022373?text=Merhaba%2C%20${encodeURIComponent(item.name)}%20fiyat%20teklifi%20almak%20istiyorum." target="_blank" rel="nofollow noopener noreferrer" class="inline-flex items-center px-3 py-1 bg-emerald-600 text-white rounded text-xs font-bold">Fiyat &amp; Bilgi</a>
            </td>
          </tr>`;
  }).join("\n");
  ...
  const tableRowsHtml = AMBALAJ_TECH_SPECS.map(spec => {
    return `        <tr>
          <th class="p-2 border font-bold text-gray-900" scope="row">${spec.kagitTuru}</th>
          <td class="p-2 border">${spec.gramaj}</td>
          <td class="p-2 border">${spec.standartEbat}</td>
          <td class="p-2 border">${spec.kullanimAlani}</td>
          <td class="p-2 border">${spec.baskiTuru}</td>
          <td class="p-2 border">${spec.gidaOzelligi}</td>
        </tr>`;
  }).join("\n");
  ...
  const blogGuidesHtml = AMBALAJ_BLOG_GUIDES.map(guide => {
    return `      <li class="space-y-2">
        <figure class="my-2"><img src="${guide.image}" alt="${guide.title} - Mavi Basım Matbaa Rehberi" width="1200" height="675" loading="lazy" decoding="async" class="w-full h-auto rounded-xl" /></figure>
...
<main class="seo-ambalaj-content max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 py-6">
  <header class="mb-8">
    ...
    ${AMBALAJ_IMAGE_MANIFEST["/images/ambalaj/ambalaj-baski-cozumleri.webp"]
      ? `<div class="ambalaj-hero-media my-4"><img src="/images/ambalaj/ambalaj-baski-cozumleri.webp" alt="Ambalaj kâğıdı baskısı, sülfit ve pelür sarım çözümleri" width="800" height="500" loading="eager" fetchpriority="high" class="w-full h-auto rounded-xl" /></div>`
      : `<div class="ambalaj-hero-placeholder my-4" style="aspect-ratio:8/5;background:#f8fafc;border:1px dashed #cbd5e1;border-radius:1rem;display:flex;align-items:center;justify-content:center;text-align:center;padding:1.5rem;"><span style="font-size:12px;color:#64748b;">Baskılı Ambalaj Çözümleri (Görsel Hazırlanıyor)</span></div>`}
  </header>
...
${productsHtml}
    ...
    <div class="mt-6">
      ...
      <table class="w-full text-left border-collapse text-xs md:text-sm my-3 border border-gray-200">
        <caption class="sr-only">Standart Ambalaj Kâğıdı Paket ve Fiyat Listesi</caption>
        <thead>
          <tr class="bg-gray-100">
            <th class="p-2 border" scope="col">KOD</th>
            <th class="p-2 border" scope="col">ÜRÜN VE KÂĞIT ÖZELLİĞİ</th>
            <th class="p-2 border" scope="col">STANDART MİKTAR</th>
            <th class="p-2 border" scope="col">FİYAT</th>
            <th class="p-2 border" scope="col">BİLGİ &amp; SİPARİŞ</th>
          </tr>
        </thead>
        <tbody>
${priceTableRowsHtml}
  ...
  <section id="malzeme-karsilastirmasi" class="mb-10">
    ...
    ${AMBALAJ_IMAGE_MANIFEST["/images/ambalaj/ambalaj-kagit-ve-malzemeleri.webp"]
      ? `<figure class="my-4"><img src="/images/ambalaj/ambalaj-kagit-ve-malzemeleri.webp" alt="Ambalaj kağıdı ve sarım malzemesi özellikleri" width="600" height="400" loading="lazy" class="w-full h-auto rounded-xl" /><figcaption class="text-xs text-gray-500 mt-1">Ambalaj Kağıdı ve Malzeme Özellikleri</figcaption></figure>`
      : `<div class="ambalaj-material-placeholder my-4" style="aspect-ratio:3/2;background:#f8fafc;border:1px dashed #cbd5e1;border-radius:0.75rem;display:flex;align-items:center;justify-content:center;text-align:center;padding:1rem;"><span style="font-size:11px;color:#64748b;">Ambalaj Kağıdı Özellikleri (Görsel Hazırlanıyor)</span></div>`}
    <h3>Kâğıt Türleri Teknik Özellikleri</h3>
    <table class="w-full text-left border-collapse text-xs md:text-sm my-3 border border-gray-200">
      <caption class="sr-only">Ambalaj Kâğıdı Türleri ve Malzeme Özellikleri Karşılaştırması</caption>
      <thead>
        <tr class="bg-gray-100">
          <th class="p-2 border" scope="col">Kâğıt Türü</th>
          <th class="p-2 border" scope="col">Gramaj</th>
          <th class="p-2 border" scope="col">Standart Ebat</th>
          <th class="p-2 border" scope="col">Öne Çıkan Kullanım Alanı</th>
          <th class="p-2 border" scope="col">Baskı Türü</th>
          <th class="p-2 border" scope="col">Kullanım &amp; Gıda Özelliği</th>
        </tr>
      </thead>
      <tbody>
${tableRowsHtml}
...
${priceFactorsHtml}
  ...
  <section id="dosya-ve-prova" class="mb-10">
    ...
    ${AMBALAJ_IMAGE_MANIFEST["/images/ambalaj/ambalaj-baski-pdf-prova.webp"]
      ? `<figure class="my-4"><img src="/images/ambalaj/ambalaj-baski-pdf-prova.webp" alt="Baskı öncesi grafik kontrolü ve dijital PDF prova süreci" width="600" height="400" loading="lazy" class="w-full h-auto rounded-xl" /><figcaption class="text-xs text-gray-500 mt-1">Baskı Öncesi Grafik Hazırlık ve PDF Prova</figcaption></figure>`
      : `<div class="ambalaj-proof-placeholder my-4" style="aspect-ratio:3/2;background:#f8fafc;border:1px dashed #cbd5e1;border-radius:0.75rem;display:flex;align-items:center;justify-content:center;text-align:center;padding:1rem;"><span style="font-size:11px;color:#64748b;">Baskı Öncesi Grafik Hazırlık (Görsel Hazırlanıyor)</span></div>`}
${fileStepsHtml}
  </section>
...
${considerationsHtml}
  ...
  <section id="ilgili-baski-urunleri" class="mb-10">
    ...
    ${AMBALAJ_IMAGE_MANIFEST["/images/ambalaj/ambalaj-baski-urunleri.webp"]
      ? `<figure class="my-4"><img src="/images/ambalaj/ambalaj-baski-urunleri.webp" alt="Ambalaj baskısını tamamlayan kurumsal matbaa ürünleri" width="600" height="400" loading="lazy" class="w-full h-auto rounded-xl" /><figcaption class="text-xs text-gray-500 mt-1">Tamamlayıcı Kurumsal Matbaa ve Ambalaj Ürünleri</figcaption></figure>`
      : `<div class="ambalaj-related-placeholder my-4" style="aspect-ratio:3/2;background:#f8fafc;border:1px dashed #cbd5e1;border-radius:0.75rem;display:flex;align-items:center;justify-content:center;text-align:center;padding:1rem;"><span style="font-size:11px;color:#64748b;">Kurumsal Ambalaj Çözümleri (Görsel Hazırlanıyor)</span></div>`}
