import { KUTU_IMAGE_MANIFEST } from '../generated/sectorImageManifest';
import { KUTU_DATA } from './productData';
...
export const KUTU_SEO_METADATA = {
  ...
  desc: "Kutu baskı ve baskılı karton kutu fiyatlarını inceleyin. Bristol veya kroma karton, özel ölçü, baskı ve yüzey seçenekleri için teklif alın.",
...
export const KUTU_FAQS: KutuFaqItem[] = [
  ...
  },
  {
    ...
    a: "Kutu baskı fiyatı; kutu ölçüsü ve açık tabaka ebadı, sipariş adedi, seçilen karton türü (Bristol/Kroma) ve gramajı (230–400 gr), baskı renk sayısı, koruyucu laminasyon türü (mat, parlak, Soft Touch) ile gofre kabartma, varak yaldız ve bölgesel lak gibi ek yüzey uygulamalarına göre belirlenir."
  },
  {
  ...
  },
  {
    ...
    a: "Standart sipariş süreçlerinde dijital PDF prova üzerinden onay alınarak ilerlenir. Grafik desteği var. Fiziksel numune, maket, özel bıçak ve kalıp ihtiyacı ise siparişin adet ve teknik özelliklerine göre teklif aşamasında ayrıca değerlendirilir."
  },
  {
...
export const KUTU_H2_SECTIONS: KutuH2Section[] = [
  {
    ...
    subtitle: "Standart model seçeneklerine göre kutu modelleri, teknik özellikler ve kurumsal teklif süreci. Kutu baskı fiyatları ve baskılı karton kutu fiyatları; ölçü, karton türü, gramaj, baskı, yüzey uygulamaları ve sipariş adedine göre belirlenir. Kutu basımı ve özel ölçülü kutu bastırma talepleri için teklif, ürün detaylarına göre hazırlanır.",
    ...
    subsections: [
      ...
      },
      {
        ...
        description: "Standart ölçülerin dışındaki kurumsal projelerde özel ölçülü baskılı kutu yaptırma süreci için en, boy ve yükseklik ölçülerine göre özel bıçak çizimi hazırlanır. Kutu içine girecek ürünün ağırlığına ve kullanım amacına göre karton türü ve gramajı değerlendirilerek uygun tiraj ve maliyet dengesine göre teklif sunulur."
      }
    ]
  },
  {
  ...
  },
  {
    ...
    subsections: [
      ...
      },
      {
        ...
        title: "Kutu Bıçak İzi Nedir? Bıçak İzi Oluşturma ve Katlama Payları",
        description: "Matbaa ve ambalaj üretiminde kutu bıçak izi nedir sorusunun karşılığı; kutunun kesim ve kırım sınırlarını gösteren vektörel teknik şablondur. Kutu kesim kalıbı; çelik kesim bıçakları (düz çizgi), pilyaj/kırım bıçakları (katlama çizgisi) ve perfore bıçaklarından oluşur. Kesim, pilyaj/kırım, yapıştırma alanları ve taşma payı ayrı biçimde belirtilmelidir. Bıçak izi oluşturma işlemi kutunun ölçüsü, kapak yapısı, taban kilidi ve yapıştırma payına göre yapılır. Müşteri hazır bıçak izi gönderemiyorsa ölçüler ve ürün bilgileri üzerinden teknik değerlendirme yapılabilir; kesin üretim uygunluğu teknik kontrol ve onay sonucunda belirlenir."
      }
    ]
  },
  {
  ...
  },
  {
    ...
    subsections: [
      ...
      },
      {
        ...
        description: "Seçilen kartonun cinsi (Bristol veya Kroma), gramajı (230 gr ile 400 gr arası) ve talep edilen selefon, varak yaldız, gofre veya lak gibi ek uygulamalar toplam teklif tutarını şekillendirir. Net kutu baskı fiyatı; kutunun açık ebadı, karton gramajı, baskı özellikleri, bıçak veya kalıp ihtiyacı ve yüzey uygulamalarına göre hesaplanır."
      }
    ]
  },
  {
    ...
    subsections: [
      {
        ...
        description: "Onaylanan tasarım dosyası belirlenen baskı yöntemine göre hazırlanır; ofset teknikle kutu üzerine baskı uygulanırken sipariş özelliklerine göre hassas renk kontrolü gerçekleştirilir."
      },
      {
...
export function generateKutuSSRBodyContent(): string {
  ...
  const sectionsHtml = KUTU_H2_SECTIONS.map((sec) => {
    ...
    if (sec.id === "baski-ornekleri") {
      galleryHtml = `\n    <div class="kutu-gallery-grid">\n${KUTU_GALLERY.map(img => {
        const isAvail = Boolean(img.img && (KUTU_IMAGE_MANIFEST as Record<string, boolean>)[img.img]);
        return `      <figure id="${img.id}">
        ${isAvail ? `<img src="${img.img}" alt="${img.title} - Mavi Basım Ambalaj" title="${img.title}" loading="lazy" decoding="async" width="450" height="300" />` : `<div class="kutu-image-placeholder" style="aspect-ratio:3/2;background:#f1f5f9;display:flex;align-items:center;justify-content:center;text-align:center;padding:1rem;"><span style="font-size:11px;color:#64748b;">${img.title} (Görsel Hazırlanıyor)</span></div>`}
    ...
    let tableHtml = "";
    if (sec.id === "fiyat-ve-teklif") {
      tableHtml = `\n    <div class="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden relative z-10 my-6">
      <div class="overflow-x-auto">
        <table class="w-full border-collapse text-[11px] md:text-[13px]">
          <caption class="sr-only">Karton kutu baskı ürünleri ve teklif tablosu</caption>
          <thead>
            <tr class="bg-black text-white border-b border-black">
              <th scope="col" class="p-4 w-24 font-black uppercase tracking-tight text-[14px] md:text-[16px] border-r border-white/10">KOD</th>
              <th scope="col" class="p-4 font-black uppercase tracking-tight text-[14px] md:text-[16px] border-r border-white/10 text-left">ÜRÜN ADI</th>
              <th scope="col" class="p-4 font-black uppercase tracking-tight text-[14px] md:text-[16px] border-r border-white/10 text-center">ÖLÇÜ</th>
              <th scope="col" class="p-4 font-black uppercase tracking-tight text-[14px] md:text-[16px] border-r border-white/10 text-left">ÖZELLİK</th>
              <th scope="col" class="p-4 font-black uppercase tracking-tight text-[14px] md:text-[16px] border-r border-white/10 text-center">MİKTAR</th>
              <th scope="col" class="p-4 font-black uppercase tracking-tight text-[14px] md:text-[16px] border-r border-white/10 text-center">FİYAT</th>
              <th scope="col" class="p-4 font-black uppercase tracking-tight text-[14px] md:text-[16px] text-center">SİPARİŞ</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-100">
${KUTU_DATA.map((item) => {
  const whatsappUrl = new URL('https://wa.me/905366022373');
  const message = `Merhaba, ${item.code} - ${item.name} (${item.olcu}, ${item.ozellik}, ${item.miktar}) için fiyat teklifi ve detaylı bilgi almak istiyorum.`;
  whatsappUrl.searchParams.set('text', message);
  return `            <tr class="hover:bg-blue-50/50 transition-colors">
              <th scope="row" class="p-3 text-center font-black text-primary border-r border-gray-100">${item.code}</th>
              <td class="p-3 font-bold text-black border-r border-gray-100">${item.name}</td>
              <td class="p-3 text-center font-semibold text-slate-700 border-r border-gray-100 whitespace-nowrap">${item.olcu}</td>
              <td class="p-3 text-slate-600 border-r border-gray-100">${item.ozellik}</td>
              <td class="p-3 text-center font-bold text-black border-r border-gray-100">${item.miktar}</td>
              <td class="p-3 text-center font-black text-black border-r border-gray-100">${item.price}</td>
              <td class="p-3 text-center">
                <a href="${whatsappUrl.toString()}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center justify-center gap-2 bg-primary hover:bg-secondary text-white px-4 py-2 rounded-full text-xs md:text-sm font-black tracking-tight leading-none transition-all shadow-md whitespace-nowrap">
      ...
      <div class="bg-slate-50 px-5 py-3 border-t border-gray-150 flex flex-col sm:flex-row justify-center items-center text-xs text-slate-500 font-medium font-sans">
    ...
    <div class="mt-4 text-center max-w-4xl mx-auto px-4">
      <p class="text-slate-500 font-semibold text-xs text-center leading-relaxed">
        * Ofset baskıda renk, kesim ve adet toleransları oluşabilir. Ayrıntılı bilgi için <a href="/teslimat-sartlari" class="underline hover:text-primary transition-colors">Teslimat Şartları</a> sayfasını inceleyebilirsiniz.
      </p>
    </div>`;
    }
    ...
    <h2>${sec.h2Title}</h2>
    <p>${sec.subtitle}</p>${tableHtml}${galleryHtml}
${subsectionsHtml}
  </section>`;
  }).join("\n\n");
...
<main class="seo-kutu-content">
  <header>
    <h1>${meta.h1}</h1>
    <p>${meta.desc}</p>
    <p>Kutu baskı ve baskılı karton kutu seçenekleri; ürün ölçüsü, kullanım amacı, karton türü ve sipariş adedine göre değerlendirilir. Özel baskılı karton kutu ve baskılı kutu ambalaj talepleri, baskı ve yüzey uygulamalarına göre tekliflendirilir.</p>
    ...
    <div class="my-6 p-4 rounded-2xl bg-slate-50 border border-slate-200/80 flex items-center gap-3 text-xs md:text-sm text-slate-700 font-medium">
      <span><strong>Türkiye geneline anlaşmalı kargo ile sevk edilmektedir:</strong> Hazırlık ve gönderim süresi; kutunun ölçüsü, sipariş adedi, karton türü, kalıp ihtiyacı, yüzey uygulamaları ve onay sürecine göre teklif aşamasında belirlenir. Detaylar için <a href="/teslimat-sartlari" class="text-primary underline font-bold">teslimat şartları</a> sayfamızı inceleyebilirsiniz.</span>
    </div>
