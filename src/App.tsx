...
export const FeatureTooltip = ({ code }: { code: string }) => {
  ...
  const getInfo = (code: string) => {
    ...
    if (code.startsWith('KC')) {
      const isMat = code.endsWith('-2');
      return `210 gr. Amerikan Bristol kağıttan üretilmiştir. ${isMat ? 'Mat' : 'Parlak'} selefon kaplıdır. Dayanıklı ve prestijli bir taşıma çözümüdür.`;
    }
...
const PRODUCTS = [
  ...
  { id: "makbuz", name: "MAKBUZ & FORMLAR", price: "1.450 ₺", desc: "5 Cilt", image: LOCAL_ASSETS.makbuz, type: "makbuz-ve-formlar" },
  ...
  return (
    <motion.div 
      ...
      onClick={() => {
        ...
        else if (product.type === "kitap-ayraci") navigate("/kitap-ayraci");
        else if (product.type === "yag-karti") navigate("/yag-karti");
...
const HomePage = () => {
  ...
  useEffect(() => {
    // Show first 18 products on the homepage grid (to include Makbuz)
    setRandomProducts(PRODUCTS.slice(0, 18));
  }, []);
