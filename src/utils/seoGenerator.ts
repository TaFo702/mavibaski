  ...
  if (p === '/amerikan-servis') {
    ...
    const amerikanServisFaqs = [
      ...
      },
      {
        ...
        a: "Amerikan servis kağıdı baskı dosyaları 300 DPI çözünürlükte, CMYK renk modunda ve kesim kenarlarından en az 3 mm taşma payı bırakılarak PDF/X formatında hazırlanmalıdır. Baskıya hazır dosyanız için teknik kontrol ve dijital PDF prova sunulmaktadır. Grafik desteği var."
      }
    ];
...
export function injectSEOMetadata(html: string, title: string, desc: string, canonicalUrl: string, is404 = false, extraHead = '', h1Text = '', bodyContent = '', ogImage?: string): string {
  ...
  const extraOgSiteMatch = processedExtraHead.match(/<meta\s+[^>]*property=["']og:site_name["']\s+content=["']([^"']+)["']\s*\/?>/i);
  const resolvedOgSite = extraOgSiteMatch ? extraOgSiteMatch[1] : 'Mavi Basım Matbaa & Reklam';
  if (/<meta\s+[^>]*property=["']og:site_name["']/i.test(result)) {
    result = result.replace(/<meta\s+[^>]*property=["']og:site_name["'][\s\S]*?\/?>/i, `<meta data-rh="true" property="og:site_name" content="${resolvedOgSite}" />`);
  } else {
    result = result.replace('</head>', `  <meta data-rh="true" property="og:site_name" content="${resolvedOgSite}" />\n</head>`);
  }
  if (extraOgSiteMatch) {
    processedExtraHead = processedExtraHead.replace(/<meta\s+[^>]*property=["']og:site_name["'][\s\S]*?\/?>\n?/gi, "");
  }

  // Replace og:locale
  const extraOgLocaleMatch = processedExtraHead.match(/<meta\s+[^>]*property=["']og:locale["']\s+content=["']([^"']+)["']\s*\/?>/i);
  const resolvedOgLocale = extraOgLocaleMatch ? extraOgLocaleMatch[1] : 'tr_TR';
  if (/<meta\s+[^>]*property=["']og:locale["']/i.test(result)) {
    result = result.replace(/<meta\s+[^>]*property=["']og:locale["'][\s\S]*?\/?>/i, `<meta data-rh="true" property="og:locale" content="${resolvedOgLocale}" />`);
  } else {
    result = result.replace('</head>', `  <meta data-rh="true" property="og:locale" content="${resolvedOgLocale}" />\n</head>`);
  }
  if (extraOgLocaleMatch) {
    processedExtraHead = processedExtraHead.replace(/<meta\s+[^>]*property=["']og:locale["'][\s\S]*?\/?>\n?/gi, "");
  }

  // If ogImage is explicitly provided or extraHead contains an og:image tag, resolve og:image
  const extraOgImageMatch = processedExtraHead.match(/<meta\s+[^>]*property=["']og:image["']\s+content=["']([^"']+)["']\s*\/?>/i);
  const resolvedOgImage = ogImage || (extraOgImageMatch ? extraOgImageMatch[1] : 'https://mavibasim.com/mavilogo.png');
  ...
  const extraOgSecureMatch = processedExtraHead.match(/<meta\s+[^>]*property=["']og:image:secure_url["']\s+content=["']([^"']+)["']\s*\/?>/i);
  const resolvedOgSecure = extraOgSecureMatch ? extraOgSecureMatch[1] : resolvedOgImage;
  result = result.replace(/<meta\s+[^>]*property=["']og:image:secure_url["'][\s\S]*?\/?>/i, `<meta data-rh="true" property="og:image:secure_url" content="${resolvedOgSecure}" />`);
  if (extraOgSecureMatch) {
    processedExtraHead = processedExtraHead.replace(/<meta\s+[^>]*property=["']og:image:secure_url["'][\s\S]*?\/?>\n?/gi, "");
  }

  // Replace og:image:type
  const extraOgTypeMatch = processedExtraHead.match(/<meta\s+[^>]*property=["']og:image:type["']\s+content=["']([^"']+)["']\s*\/?>/i);
  const defaultType = resolvedOgImage.endsWith('.webp') ? 'image/webp' : (resolvedOgImage.endsWith('.png') ? 'image/png' : 'image/jpeg');
  const resolvedOgType = extraOgTypeMatch ? extraOgTypeMatch[1] : defaultType;
  result = result.replace(/<meta\s+[^>]*property=["']og:image:type["'][\s\S]*?\/?>/i, `<meta data-rh="true" property="og:image:type" content="${resolvedOgType}" />`);
  if (extraOgTypeMatch) {
    processedExtraHead = processedExtraHead.replace(/<meta\s+[^>]*property=["']og:image:type["'][\s\S]*?\/?>\n?/gi, "");
  }

  // Replace og:image:width
  const extraOgWidthMatch = processedExtraHead.match(/<meta\s+[^>]*property=["']og:image:width["']\s+content=["']([^"']+)["']\s*\/?>/i);
  const defaultWidth = resolvedOgImage.endsWith('.webp') ? '800' : '1200';
  const resolvedOgWidth = extraOgWidthMatch ? extraOgWidthMatch[1] : defaultWidth;
  result = result.replace(/<meta\s+[^>]*property=["']og:image:width["'][\s\S]*?\/?>/i, `<meta data-rh="true" property="og:image:width" content="${resolvedOgWidth}" />`);
  if (extraOgWidthMatch) {
    processedExtraHead = processedExtraHead.replace(/<meta\s+[^>]*property=["']og:image:width["'][\s\S]*?\/?>\n?/gi, "");
  }

  // Replace og:image:height
  const extraOgHeightMatch = processedExtraHead.match(/<meta\s+[^>]*property=["']og:image:height["']\s+content=["']([^"']+)["']\s*\/?>/i);
  const defaultHeight = resolvedOgImage.endsWith('.webp') ? '533' : '630';
  const resolvedOgHeight = extraOgHeightMatch ? extraOgHeightMatch[1] : defaultHeight;
  result = result.replace(/<meta\s+[^>]*property=["']og:image:height["'][\s\S]*?\/?>/i, `<meta data-rh="true" property="og:image:height" content="${resolvedOgHeight}" />`);
  if (extraOgHeightMatch) {
    processedExtraHead = processedExtraHead.replace(/<meta\s+[^>]*property=["']og:image:height["'][\s\S]*?\/?>\n?/gi, "");
  }

  // Replace og:image:alt
  const extraOgAltMatch = processedExtraHead.match(/<meta\s+[^>]*property=["']og:image:alt["']\s+content=["']([^"']+)["']\s*\/?>/i);
  const resolvedOgAlt = extraOgAltMatch ? extraOgAltMatch[1] : cleanTitle;
  result = result.replace(/<meta\s+[^>]*property=["']og:image:alt["'][\s\S]*?\/?>/i, `<meta data-rh="true" property="og:image:alt" content="${resolvedOgAlt}" />`);
  if (extraOgAltMatch) {
    processedExtraHead = processedExtraHead.replace(/<meta\s+[^>]*property=["']og:image:alt["'][\s\S]*?\/?>\n?/gi, "");
  }

  // Replace twitter:image
  const extraTwitterImageMatch = processedExtraHead.match(/<meta\s+[^>]*name=["']twitter:image["']\s+content=["']([^"']+)["']\s*\/?>/i);
  const resolvedTwitterImage = extraTwitterImageMatch ? extraTwitterImageMatch[1] : resolvedOgImage;
  result = result.replace(/<meta\s+[^>]*name=["']twitter:image["'][\s\S]*?\/?>/i, `<meta data-rh="true" name="twitter:image" content="${resolvedTwitterImage}" />`);
  if (extraTwitterImageMatch) {
    processedExtraHead = processedExtraHead.replace(/<meta\s+[^>]*name=["']twitter:image["'][\s\S]*?\/?>\n?/gi, "");
  }

  // Clean any redundant twitter tags or duplicate tags from extraHead
  processedExtraHead = processedExtraHead.replace(/<meta\s+[^>]*name=["']twitter:card["'][\s\S]*?\/?>\n?/gi, "");
  processedExtraHead = processedExtraHead.replace(/<meta\s+[^>]*name=["']twitter:title["'][\s\S]*?\/?>\n?/gi, "");
  processedExtraHead = processedExtraHead.replace(/<meta\s+[^>]*name=["']twitter:description["'][\s\S]*?\/?>\n?/gi, "");
