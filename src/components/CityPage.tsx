...
export const CityPage: React.FC<{ citySlug?: string }> = ({ citySlug: propCitySlug }) => {
  ...
  const isSafeCity = city.slug === "adana-matbaa" || city.slug === "antalya-matbaa" || city.slug === "mersin-matbaa" || city.slug === "osmaniye-matbaa" || city.slug === "ankara-matbaa" || city.slug === "izmir-matbaa" || city.slug === "ordu-matbaa" || city.slug === "samsun-matbaa" || city.slug === "sinop-matbaa" || city.slug === "amasya-matbaa" || city.slug === "rize-matbaa" || city.slug === "trabzon-matbaa";

  const getDativeSuffix = (name: string) => {
    const vowels = name.match(/[aıoueiöü]/gi);
    const lastVowel = vowels ? vowels[vowels.length - 1].toLowerCase() : 'a';
    const isBack = ['a', 'ı', 'o', 'u'].includes(lastVowel);
    const endsWithVowel = /[aıoueiöü]$/i.test(name);
    if (endsWithVowel) {
      return isBack ? '’ya' : '’ye';
    } else {
      return isBack ? '’a' : '’e';
  ...
  const getLocativeSuffix = (name: string) => {
    const vowels = name.match(/[aıoueiöü]/gi);
    const lastVowel = vowels ? vowels[vowels.length - 1].toLowerCase() : 'a';
    const isBack = ['a', 'ı', 'o', 'u'].includes(lastVowel);
    const endsWithVoiceless = /[çfhksştp]$/i.test(name.trim());
    const consonant = endsWithVoiceless ? 't' : 'd';
    const vowel = isBack ? 'a' : 'e';
    return `’${consonant}${vowel}ki`;
  };
  ...
  const safeWorkflowSteps = [
    { step: "01", title: "Ürün ve Adet Seçimi", desc: "İhtiyacınıza uygun ürün grubu, ebat, adet ve kullanım amacını belirleyin." },
    { step: "02", title: "Tasarım İletimi", desc: "Tasarım dosyasının iletilmesi veya tasarım ihtiyacının paylaşılması." },
    { step: "03", title: "Teknik Dosya Kontrolü", desc: "İletilen dosyanın teknik açıdan incelenmesi ve kontrolü." },
    { step: "04", title: "PDF Prova Süreci", desc: "Uygun işlerde yerleşim ve metin kontrolü için PDF prova paylaşılması." },
    { step: "05", title: "Baskı Onayı", desc: "Paylaşılan prova veya dosya üzerinden yazılı baskı onayının verilmesi." },
    { step: "06", title: "Üretim Planı", desc: "Onaylanan çalışma için üretim planının oluşturulması." },
    { step: "07", title: "Kargo Planı ve Gönderim Bilgisi", desc: "Kargo planı ve gönderim bilgisinin netleştirilerek paylaşılması." }
  ];

  const workflowSteps = isSafeCity ? safeWorkflowSteps : defaultWorkflowSteps;

  if (priorityContent) {
    const priorityServiceSchema = {
      "@context": "https://schema.org",
      "@type": "Service",
      "@id": `https://mavibasim.com/${priorityContent.slug}#service`,
      "name": `${priorityContent.name} Matbaa ve Baskı Hizmetleri`,
      "serviceType": "Matbaa ve Baskı Hizmetleri",
      "provider": {
        "@type": "LocalBusiness",
      ...
      "areaServed": {
        ...
        "name": priorityContent.name
      },
      "url": `https://mavibasim.com/${priorityContent.slug}`,
      "description": priorityContent.metaDescription
    };
    ...
    return (
      <>
        <Helmet>
          ...
          <meta property="og:url" content={`https://mavibasim.com/${priorityContent.slug}`} />
          ...
          <script type="application/ld+json">
            {JSON.stringify({
              "@context": "https://schema.org",
              "@type": "FAQPage",
              "mainEntity": priorityContent.faq.map(f => ({
                "@type": "Question",
                "name": f.question,
                "acceptedAnswer": {
                  "@type": "Answer",
        ...
        <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 pt-4 pb-12">
          ...
          <div className="bg-white rounded-2xl border border-slate-200/90 p-6 md:p-8 mb-8 shadow-sm relative overflow-hidden">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 pb-6 border-b border-slate-100">
                ...
                <div className="flex items-center gap-2.5 flex-wrap mb-3">
                  {isSafeCity ? (
                    <span className="bg-primary/10 text-primary border border-primary/20 text-xs font-black px-3 py-1 rounded-full uppercase tracking-wider flex items-center gap-1.5">
                      <Truck size={14} />
                      {priorityContent.name}{getDativeSuffix(priorityContent.name)} Kargo ile Gönderim
                    </span>
                  ) : (
                    <>
                      <span className="bg-primary/10 text-primary border border-primary/20 text-xs font-black px-3 py-1 rounded-full uppercase tracking-wider flex items-center gap-1.5">
              ...
              <div className="flex flex-col sm:flex-row gap-3 shrink-0">
                <a
                  ...
                  className="inline-flex items-center justify-center gap-2 bg-[#25D366] hover:bg-[#20ba5a] text-white font-black text-sm px-5 py-3 rounded-xl transition-all shadow-md hover:scale-105 active:scale-95"
                >
            ...
            <div className="mt-6 text-slate-700 font-semibold text-sm md:text-base leading-relaxed space-y-4">
              ...
              <div className="p-4 bg-slate-50 rounded-xl border border-slate-200/80 text-xs md:text-sm text-slate-800 font-bold flex items-start gap-3">
                ...
                <span>
                  {isSafeCity ? (
                    "Sipariş öncesinde ürün, adet, dosya durumu ve üretim planı değerlendirilir. Üretim ve kargo planı; ürünün özelliklerine, onay sürecine ve seçilen hizmetlere göre teklif aşamasında netleştirilir."
                  ) : (
                    <>
            ...
            {isSafeCity ? (
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-6 pt-6 border-t border-slate-100 text-xs font-bold text-slate-800">
                <div className="flex items-center gap-2 bg-slate-50 p-2.5 rounded-xl border border-slate-100">
                  <FileCheck2 className="text-primary w-4 h-4 shrink-0" />
                  <span>Teknik Dosya Kontrolü</span>
                </div>
                <div className="flex items-center gap-2 bg-slate-50 p-2.5 rounded-xl border border-slate-100">
                  <BadgeCheck className="text-emerald-600 w-4 h-4 shrink-0" />
                  <span>Ücretsiz PDF Prova</span>
                </div>
                <div className="flex items-center gap-2 bg-slate-50 p-2.5 rounded-xl border border-slate-100">
                  <Truck className="text-primary w-4 h-4 shrink-0" />
                  <span>{priorityContent.name}{getDativeSuffix(priorityContent.name)} Kargo ile Gönderim</span>
                </div>
                <div className="flex items-center gap-2 bg-slate-50 p-2.5 rounded-xl border border-slate-100">
                  <ShieldCheck className="text-emerald-600 w-4 h-4 shrink-0" />
                  <span>Ürüne Göre Teklif</span>
                </div>
              </div>
            ) : (
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-6 pt-6 border-t border-slate-100 text-xs font-bold text-slate-800">
          ...
          <div className="mb-12">
            <div className="mb-6">
              <h2 className="text-xl md:text-2xl font-black text-slate-900 uppercase tracking-tight flex items-center gap-2">
                ...
                <span>
                  {isSafeCity ? `${priorityContent.name}${getLocativeSuffix(priorityContent.name)} İşletmeler İçin Baskı İhtiyaçları` : `${priorityContent.name} Sektörlerine Özel Baskı İhtiyaçları`}
                </span>
              </h2>
              <p className="text-xs md:text-sm font-semibold text-slate-500 mt-1">
                {isSafeCity
                  ? `${priorityContent.name}${getLocativeSuffix(priorityContent.name)} işletmelerin ürün, adet, kullanım amacı ve dosya durumuna göre değerlendirebileceği baskı seçenekleri.`
          ...
          <div className="mb-12">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {priorityContent.featuredProducts.map((prod, idx) => {
                const categoryBadge = prod.categoryBadge || `${priorityContent.name} Özel`;
                ...
                return (
                  ...
                  >
                    <div>
                      {prod.imageSrc ? (
                        <div className="relative w-full aspect-[16/10] mb-3.5 overflow-hidden rounded-xl bg-slate-100 border border-slate-200/80">
                          <img
                            src={prod.imageSrc}
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6 md:p-8 mb-12 shadow-sm">
            <h2 className="text-lg md:text-xl font-black text-slate-900 uppercase tracking-tight mb-2">
              {isSafeCity ? `${priorityContent.name}${getDativeSuffix(priorityContent.name)} Kargo ile Gönderim` : `${priorityContent.name} İlçe ve Bölge Sevkiyat Bilgisi`}
            </h2>
            <p className="text-slate-600 font-semibold text-xs md:text-sm leading-relaxed mb-4">
              {isSafeCity
                ? `${priorityContent.name} il merkezi ve çevre ilçelerdeki işletmeler için sipariş ve kargo planı, ürünün özelliklerine ve onay sürecine göre oluşturulur.`
          ...
          <div className="bg-slate-900 text-white rounded-2xl p-6 md:p-8 mb-12 shadow-md">
            <div className="mb-6 text-center max-w-2xl mx-auto">
              ...
              <h2 className="text-xl md:text-2xl font-black uppercase tracking-tight">
                {isSafeCity ? `${priorityContent.name} Sipariş ve Kargo Süreci` : `Siparişten ${priorityContent.name} Teslimatına 7 Adımlı Süreç`}
              </h2>
              <p className="text-xs md:text-sm font-semibold text-slate-300 mt-1">
                {isSafeCity ? "Sipariş ve kargo planı adım adım şeffaf şekilde yönetilir." : "Tüm siparişleriniz adım adım kontrollü şekilde üretilip kargolanır."}
              </p>
            </div>
            ...
            {isSafeCity && (
              <div className="mt-6 pt-4 border-t border-slate-800 text-xs text-slate-400 space-y-1.5">
                <p>
                  <strong className="text-slate-300">PDF prova açıklaması:</strong> PDF prova, yerleşim ve metin kontrolüne yardımcı olur. Fiziksel baskının renk, kâğıt dokusu veya karton sertliğini birebir temsil etmez.
                </p>
                <p>
                  <strong className="text-slate-300">Kargo açıklaması:</strong> {priorityContent.name}{getDativeSuffix(priorityContent.name)} gönderim planı; ürün, adet, baskı sonrası işlemler ve sipariş onayına göre teklif aşamasında netleştirilir.
                </p>
              </div>
            )}
          </div>
            ...
            <div className="mb-12">
              <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
                ...
                <p className="text-slate-600 font-semibold text-xs md:text-sm leading-relaxed">
                  Daha net teklif için ürün türünü, ölçüyü, adedi, baskı yönünü, varsa malzeme veya baskı sonrası tercihini ve tasarım dosyanızın durumunu paylaşabilirsiniz.
                </p>
              </div>
            </div>
          ) : (
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6 md:p-8 mb-12 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-stretch">
              {priorityContent.faq.map((f, idx) => (
                <div key={idx} className="h-full bg-slate-50/80 p-5 rounded-xl border border-slate-100 flex flex-col justify-between">
                  <div>
  ...
  const defaultServiceSchema = {
    "@context": "https://schema.org",
    "@type": "Service",
    "@id": `https://mavibasim.com/${city.slug}#service`,
    "name": `${city.name} Matbaa Baskı Hizmetleri`,
    "serviceType": "Matbaa ve Baskı Hizmetleri",
    "provider": {
      "@type": "LocalBusiness",
      "name": "Mavi Basım Matbaa & Reklam",
    ...
    "description": `İstanbul Topkapı üretim tesisimizden ${city.name} ve tüm ilçelerine kargo ile kartvizit, broşür, etiket, kutu, ambalaj ve kurumsal matbaa baskı hizmeti.`
  };
  ...
  return (
    <>
      <Helmet>
        ...
        <meta property="og:url" content={`https://mavibasim.com/${city.slug}`} />
        <script type="application/ld+json">
          {JSON.stringify(defaultServiceSchema)}
        </script>
        <script type="application/ld+json">
          {JSON.stringify(defaultFaqSchema)}
        </script>
      </Helmet>
      ...
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 pt-4 pb-12">
        ...
        <div className="bg-white rounded-2xl border border-slate-200/90 p-6 md:p-8 mb-8 shadow-sm relative overflow-hidden">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-6 pt-6 border-t border-slate-100 text-xs font-bold text-slate-800">
            ...
            <div className="flex items-center gap-2 bg-slate-50 p-2.5 rounded-xl border border-slate-100">
              <ShieldCheck className="text-emerald-600 w-4 h-4 shrink-0" />
              <span>Doğrudan Üretici Fiyatı</span>
            </div>
          </div>
        </div>
