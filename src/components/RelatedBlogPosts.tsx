...
export const RelatedBlogPosts: React.FC<RelatedBlogPostsProps> = ({
  ...
  // Aynı görselin blog kartlarında ve ürün galerisinde tekrar etmesini önle
const excludedImageSet = new Set(
  excludeImages.map((image) => image.trim())
);

const fallbackPosts = BLOG_POSTS.filter(
  (post) => !matchedPosts.includes(post)
);

const finalPosts = [...matchedPosts, ...fallbackPosts]
  .filter((post, index, posts) => {
    const image = post.image?.trim();

    if (!image || excludedImageSet.has(image)) {
      return false;
    }

    return (
      posts.findIndex(
        (candidate) => candidate.image?.trim() === image
      ) === index
    );
  })
  .slice(0, 3);
