...
export const Kutu = () => {
  ...
  React.useEffect(() => {
    document.title = KUTU_SEO_METADATA.title;
    // Clean up SSR data-rh tags so client-hoisted tags do not duplicate
    const ssrRhTags = document.querySelectorAll('head > [data-rh="true"]');
    ssrRhTags.forEach(el => el.remove());

    const schemasToManage = [
      { id: 'kutu-breadcrumb-schema', data: KUTU_SCHEMAS.breadcrumbSchema },
      { id: 'kutu-service-schema', data: KUTU_SCHEMAS.serviceSchema },
      { id: 'kutu-faq-schema', data: KUTU_SCHEMAS.faqSchema },
    ];
  ...
  return (
    <div className="bg-white min-h-screen pb-20">
      <Helmet>
        ...
        <meta name="twitter:title" content={KUTU_SEO_METADATA.title} />
        <meta name="twitter:description" content={KUTU_SEO_METADATA.desc} />
        <meta name="twitter:image" content="https://mavibasim.com/images/kutu/ozel-kesim-kutu-tasarimi.webp" />
      </Helmet>
      ...
      <div className="max-w-[1320px] mx-auto px-4 sm:px-6 lg:px-8 mt-2 animate-in fade-in duration-500">
        <CategoryHero
          title={KUTU_SEO_METADATA.h1}
          badge="Amerikan Bristol / Kroma Karton Kesim"
          description={
            <p>
              Amerikan Bristol ve Kroma Kartondan özel kesimli karton kutular hazırlıyoruz. Kutu baskı ve baskılı karton kutu seçenekleri; ürün ölçüsü, kullanım amacı, karton türü ve sipariş adedine göre değerlendirilir. Özel baskılı karton kutu ve baskılı kutu ambalaj talepleri, baskı ve yüzey uygulamalarına göre tekliflendirilir. Kurumsal ambalaj takımınızı tamamlamak için <Link to="/etiket" className="text-primary hover:underline font-bold">yapışkanlı etiket</Link>, <Link to="/karton-canta" className="text-primary hover:underline font-bold">karton çanta</Link>, <Link to="/ambalaj" className="text-primary hover:underline font-bold">ambalaj baskı</Link>, <Link to="/brosur" className="text-primary hover:underline">broşür</Link> ve <Link to="/kartvizit" className="text-primary hover:underline">kartvizit</Link> ürünlerimizi inceleyebilirsiniz.
            </p>
          }
          relatedLinks={[
        ...
        <section id="fiyat-ve-teklif" className="mt-8">
          ...
          <div className="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden relative z-10 mr-16 sm:mr-20 xl:mr-0">
            <div className="overflow-x-auto">
              <table className="w-full border-collapse text-[11px] md:text-[13px]">
                <caption className="sr-only">
                  Karton kutu baskı ürünleri ve teklif tablosu
                </caption>
                <thead>
                  <tr className="bg-black text-white border-b border-black">
                    ...
                    <th scope="col" className="p-4 font-black uppercase tracking-tight text-[14px] md:text-[16px] border-r border-white/10 text-left">ÜRÜN ADI</th>
                    <th scope="col" className="p-4 font-black uppercase tracking-tight text-[14px] md:text-[16px] border-r border-white/10 text-center">ÖLÇÜ</th>
                    <th scope="col" className="p-4 font-black uppercase tracking-tight text-[14px] md:text-[16px] border-r border-white/10 text-left">ÖZELLİK</th>
                    <th scope="col" className="p-4 font-black uppercase tracking-tight text-[14px] md:text-[16px] border-r border-white/10 text-center">MİKTAR</th>
                    <th scope="col" className="p-4 font-black uppercase tracking-tight text-[14px] md:text-[16px] border-r border-white/10 text-center">FİYAT</th>
                    <th scope="col" className="p-4 font-black uppercase tracking-tight text-[14px] md:text-[16px] text-center">SİPARİŞ</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {KUTU_DATA.map((item, idx) => (
                    <tr key={idx} className="hover:bg-blue-50/50 transition-colors">
                      <th scope="row" className="p-3 text-center font-black text-primary border-r border-gray-100">
  {item.code}
</th>
          ...
          <div className="mt-4 text-center max-w-4xl mx-auto px-4">
            <p className="text-slate-500 font-semibold text-xs text-center leading-relaxed">
              * Ofset baskıda renk, kesim ve adet toleransları oluşabilir. Ayrıntılı bilgi için <Link to="/teslimat-sartlari" className="underline hover:text-primary transition-colors">Teslimat Şartları</Link> sayfasını inceleyebilirsiniz.
            </p>
          </div>
        ...
        <section id="baski-ornekleri" className="mt-16 bg-white p-6 sm:p-10 rounded-[32px] border border-slate-200 shadow-xs">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {KUTU_GALLERY.map((imgItem) => {
              const isImageAvailable = Boolean(imgItem.img && (KUTU_IMAGE_MANIFEST as Record<string, boolean>)[imgItem.img]);
              return (
                <div 
                  key={imgItem.id} 
...
height={300}
                        ...
                        <div
                          ...
                          className="w-full h-full border border-dashed border-slate-300 flex flex-col items-center justify-center p-4 text-center bg-slate-100"
                        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            <article id="ofset-baski-sureci" className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs">
              ...
              <p className="text-xs text-slate-600 font-medium leading-relaxed">
                Onaylanan tasarım dosyası belirlenen baskı yöntemine göre hazırlanır; ofset teknikle kutu üzerine baskı uygulanırken sipariş özelliklerine göre hassas renk kontrolü gerçekleştirilir.
              </p>
            </article>
