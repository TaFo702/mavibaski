...
async function runMagnetValidation() {
  console.log('🚀 STARTING PLAYWRIGHT BROWSER DOM & RESPONSIVE VALIDATION FOR /magnet...');
  ...
  try {
    ...
    await page.goto(`http://127.0.0.1:${PORT}/magnet`, { waitUntil: 'networkidle' });
  ...
  if (!allPassed) {
    ...
    console.log('✅ Magnet validation file passed.');
    process.exit(0);
  }
}

if (process.argv[1] && process.argv[1].includes('validate-magnet-playwright.ts')) {
  runMagnetValidation();
}
