...
const ROUTES_TO_TEST: RouteTestConfig[] = [
  ...
  { path: '/kup-bloknot', expectedStatus: 200, expectedTitleSubstring: 'Küp Bloknot Baskı Fiyatları | Logolu Promosyon Küp Bloknot' },
...
function validateWebpBuffer(buf: Buffer): boolean {
...
async function validateProductionHTTP() {
  ...
  // Real WebP Image Verification under the Permanent Image Policy Contract
  ...
  const manifestHasAllKeys = manifestKeys.length === 6;
  console.log(` - Real Sector Img Keys in Manifest: ${manifestKeys.length} ${manifestHasAllKeys ? '✅ (Exact 6 real images registered)' : '🔴'}`);

  const expectedImages = [
    { name: 'e-ticaret-perakende-baski-cozumleri.webp', w: 1200, h: 800, loading: 'eager', fetchPriority: 'high', isLcp: true },
    { name: 'e-ticaret-urun-kutusu-kargo-paketleme.webp', w: 900, h: 650, loading: 'lazy', isLcp: false },
    { name: 'saten-polyester-ipli-karton-canta.webp', w: 900, h: 650, loading: 'lazy', isLcp: false },
    { name: 'baskili-ambalaj-kagidi-tesekkur-karti.webp', w: 900, h: 650, loading: 'lazy', isLcp: false },
    { name: 'barkod-koli-adres-urun-etiketleri.webp', w: 900, h: 650, loading: 'lazy', isLcp: false },
    { name: 'amerikan-servis-perakende-baski-urunleri.webp', w: 900, h: 650, loading: 'lazy', isLcp: false },
  ];

  // Inspect SSR HTML for image tags and preloads
  const ssrImgs = Array.from(eTicaretDoc.querySelectorAll('img'));
  const ssrPreloads = Array.from(eTicaretDoc.querySelectorAll('link[rel="preload"]'));

  let allImageContractsPassed = manifestHasAllKeys;
  ...
  for (let idx = 0; idx < expectedImages.length; idx++) {
    ...
    const isManifestTrue = eTicaretManifest[exp.name] === true;

    console.log(`   [Görsel #${idx + 1}] ${exp.name}`);
    console.log(`      Dimensions: ${exp.w}x${exp.h} | Loading: "${exp.loading}", FetchPriority: "${exp.fetchPriority || 'none'}"`);
    console.log(`      Manifest Status: ${isManifestTrue ? 'true (Active WebP Image)' : 'false (Awaiting manual user upload)'}`);

    if (isManifestTrue) {
      // 1. MANIFEST TRUE CONTRACT:
      // - SSR must render expected <img> tag
      // - HTTP fetch of the image must return status 200 with Content-Type image/webp
      // - Binary payload must satisfy WebP integrity check
      // - Non-LCP images must not trigger unnecessary preloads
      ...
      const imgBuf = Buffer.from(await imgHttpRes.arrayBuffer());
      const isValidWebp = validateWebpBuffer(imgBuf);

      const matchingImg = ssrImgs.find(img => (img.getAttribute('src') || '').includes(exp.name));
      const ssrRendered = !!matchingImg;
      const matchingPreload = ssrPreloads.find(p => (p.getAttribute('href') || '').includes(exp.name));
      const unnecessaryPreload = !exp.isLcp && !!matchingPreload;

      const singlePass = imgStatus && typeMatch && isValidWebp && ssrRendered && !unnecessaryPreload;
      if (!singlePass) allImageContractsPassed = false;

      console.log(`      -> HTTP Status: ${imgHttpRes.status}, Content-Type: "${contentType}" ${imgStatus && typeMatch ? '✅' : '🔴'}`);
      console.log(`      -> WebP Binary Integrity: ${isValidWebp ? 'VALID ✅' : 'INVALID/CORRUPT 🔴'}`);
      console.log(`      -> SSR <img> Tag Rendered: ${ssrRendered ? 'YES ✅' : 'MISSING 🔴'}`);
      if (unnecessaryPreload) {
        console.log(`      -> Preload Check: UNNECESSARY PRELOAD FOUND 🔴`);
      }
    } else {
      // 2. MANIFEST FALSE CONTRACT:
      // - Normal and accepted state: manual image upload is pending.
      // - SSR MUST NOT render <img>, src, srcset, <picture>, preload, or CSS background for this image.
      // - Must not trigger automated HTTP image requests.
      const leakedInImg = ssrImgs.some(img => {
        const s = img.getAttribute('src') || '';
        const ss = img.getAttribute('srcset') || '';
        return s.includes(exp.name) || ss.includes(exp.name);
      });
      const leakedInPreload = ssrPreloads.some(p => (p.getAttribute('href') || '').includes(exp.name));
      const leakedInBody = eTicaretBody.includes(exp.name);

      const noLeakage = !leakedInImg && !leakedInPreload && !leakedInBody;
      if (!noLeakage) {
        allImageContractsPassed = false;
        console.log(`      -> Leakage Check: FAILED (Image referenced in SSR despite false manifest) 🔴`);
        if (leakedInImg) console.log(`         Found in <img> tag!`);
        if (leakedInPreload) console.log(`         Found in <link rel="preload">!`);
        if (leakedInBody) console.log(`         Found in HTML body text/attributes!`);
      } else {
        console.log(`      -> SSR Leakage Check: No <img>, no preload, no leaky references (Contract Satisfied) ✅`);
      }
    }
  }

  if (!manifestHasAllKeys || !allImageContractsPassed) {
    totalPassed = -1; // Contract violation
  }
  ...
  // =========================================================================
  // DEEP METADATA & SCHEMA AUDIT FOR /kup-bloknot
  // =========================================================================
  console.log('\n🔍 RUNNING DEEP METADATA, PROHIBITED WORDS & SCHEMA AUDIT FOR /kup-bloknot...');
  const kupRes = await fetch(`http://localhost:${TEST_PORT}/kup-bloknot`);
  const kupBody = await kupRes.text();
  const kupDom = new JSDOM(kupBody);
  const kupDoc = kupDom.window.document;

  const expectedKupTitle = 'Küp Bloknot Baskı Fiyatları | Logolu Promosyon Küp Bloknot';
  const expectedKupDesc = 'Küp bloknot baskı fiyatlarını, logolu ve promosyon küp bloknot seçeneklerini inceleyin. 250 ve 500 yapraklı masaüstü modeller için teklif alın.';
  const expectedKupCanonical = 'https://mavibasim.com/kup-bloknot';
  const expectedKupH1 = 'Küp Bloknot Baskı Fiyatları ve Logolu Promosyon Küp Bloknot';

  const kupTitles = Array.from(kupDoc.querySelectorAll('title'));
  const kupTitle = kupDoc.querySelector('title')?.textContent?.trim();
  const kupDescriptions = Array.from(kupDoc.querySelectorAll('meta[name="description"]'));
  const kupDesc = kupDoc.querySelector('meta[name="description"]')?.getAttribute('content')?.trim();
  const kupCanonicals = Array.from(kupDoc.querySelectorAll('link[rel="canonical"]'));
  const kupCanonical = kupDoc.querySelector('link[rel="canonical"]')?.getAttribute('href')?.trim();
  const kupRobots = kupDoc.querySelector('meta[name="robots"]')?.getAttribute('content')?.trim() || '';
  const kupH1s = Array.from(kupDoc.querySelectorAll('h1'));
  const kupH1Count = kupH1s.length;
  const kupH1Text = kupDoc.querySelector('h1')?.textContent?.trim();

  const kupOgTitle = kupDoc.querySelector('meta[property="og:title"]')?.getAttribute('content')?.trim();
  const kupOgDesc = kupDoc.querySelector('meta[property="og:description"]')?.getAttribute('content')?.trim();
  const kupOgUrl = kupDoc.querySelector('meta[property="og:url"]')?.getAttribute('content')?.trim();
  const kupTwTitle = kupDoc.querySelector('meta[name="twitter:title"]')?.getAttribute('content')?.trim();
  const kupTwDesc = kupDoc.querySelector('meta[name="twitter:description"]')?.getAttribute('content')?.trim();

  const kupJsonLdScripts = Array.from(kupDoc.querySelectorAll('script[type="application/ld+json"]'));
  const kupJsonLdTypes: string[] = [];
  let kupProductObj: any = null;
  let kupFaqQuestionsCount = 0;

  kupJsonLdScripts.forEach(script => {
    try {
      const data = JSON.parse(script.textContent || '{}');
      const items = Array.isArray(data) ? data : [data];
      items.forEach(item => {
        if (item['@type']) {
          kupJsonLdTypes.push(item['@type']);
          if (item['@type'] === 'Product') {
            kupProductObj = item;
          }
          if (item['@type'] === 'FAQPage' && item.mainEntity) {
            kupFaqQuestionsCount = item.mainEntity.length;
          }
        }
      });
    } catch {
  ...
  console.log(` - HTTP Status: ${kupRes.status} ${kupRes.status === 200 ? '✅' : '🔴'}`);
  console.log(` - Single Title (1): "${kupTitle}" ${kupTitles.length === 1 && kupTitle === expectedKupTitle ? '✅' : '🔴'}`);
  console.log(` - Single Meta Description (1): "${kupDesc}" ${kupDescriptions.length === 1 && kupDesc === expectedKupDesc ? '✅' : '🔴'}`);
  console.log(` - Single Canonical (1): "${kupCanonical}" ${kupCanonicals.length === 1 && kupCanonical === expectedKupCanonical ? '✅' : '🔴'}`);
  console.log(` - Robots Tag: "${kupRobots}" ${kupRobots.includes('index') && kupRobots.includes('follow') ? '✅' : '🔴'}`);
  console.log(` - Single H1 (1): "${kupH1Text}" ${kupH1Count === 1 && kupH1Text === expectedKupH1 ? '✅' : '🔴'}`);
  console.log(` - Open Graph Title: "${kupOgTitle}" ${kupOgTitle === expectedKupTitle ? '✅' : '🔴'}`);
  console.log(` - Open Graph Description: "${kupOgDesc}" ${kupOgDesc === expectedKupDesc ? '✅' : '🔴'}`);
  console.log(` - Open Graph URL: "${kupOgUrl}" ${kupOgUrl === expectedKupCanonical ? '✅' : '🔴'}`);
  console.log(` - Twitter Title: "${kupTwTitle}" ${kupTwTitle === expectedKupTitle ? '✅' : '🔴'}`);
  console.log(` - Twitter Description: "${kupTwDesc}" ${kupTwDesc === expectedKupDesc ? '✅' : '🔴'}`);
  console.log(` - JSON-LD Types Found: [${kupJsonLdTypes.join(', ')}] ${kupJsonLdTypes.includes('BreadcrumbList') && kupJsonLdTypes.includes('Product') && kupJsonLdTypes.includes('FAQPage') ? '✅' : '🔴'}`);
  console.log(` - Product Schema Prices: lowPrice=${kupProductObj?.offers?.lowPrice}, highPrice=${kupProductObj?.offers?.highPrice} ${Number(kupProductObj?.offers?.lowPrice) === 8350 && Number(kupProductObj?.offers?.highPrice) === 17100 ? '✅' : '🔴'}`);
  const hasNoKupAvailability = kupProductObj?.offers?.availability === undefined;
  console.log(` - Product Schema No Unverified Availability: ${hasNoKupAvailability ? 'PASSED ✅' : 'FAILED (Unverified stock found) 🔴'}`);
  const hasNoKupPriceValidUntil = kupProductObj?.offers?.priceValidUntil === undefined;
  console.log(` - Product Schema No Unverified priceValidUntil: ${hasNoKupPriceValidUntil ? 'PASSED ✅' : 'FAILED (Unverified priceValidUntil found) 🔴'}`);
  const hasKupCorrectMinOrder = kupBody.includes('Minimum sipariş adedi 100 adettir') && !kupBody.includes('Minimum sipariş adedi: 1.000 adettir');
  console.log(` - Minimum Order Text (100 adettir): ${hasKupCorrectMinOrder ? 'PASSED ✅' : 'FAILED 🔴'}`);
  console.log(` - FAQPage Questions Count: ${kupFaqQuestionsCount} ${kupFaqQuestionsCount === 8 ? '✅ (Exact 8 questions)' : '🔴'}`);
  ...
  const foundKupProhibited = kupProhibited.filter(w => 
    kupBody.toLowerCase().includes(w.toLowerCase())
  );
  console.log(` - Prohibited & Certainty Claims Check: ${foundKupProhibited.length === 0 ? 'PASSED (0 found) ✅' : `FAILED (Found: ${foundKupProhibited.join(', ')}) 🔴`}`);

  const hasKupGroupDivider = kupBody.includes('border-top: 4px solid #cbd5e1') || kupBody.includes('border-t-4 border-slate-300');
  console.log(` - Group Divider in Price Table: ${hasKupGroupDivider ? 'PASSED ✅' : 'FAILED 🔴'}`);

  if (
    kupRes.status !== 200 ||
    kupTitles.length !== 1 ||
    kupTitle !== expectedKupTitle ||
    kupDescriptions.length !== 1 ||
    kupDesc !== expectedKupDesc ||
    kupCanonicals.length !== 1 ||
    kupCanonical !== expectedKupCanonical ||
    kupH1Count !== 1 ||
    kupH1Text !== expectedKupH1 ||
    !kupRobots.includes('index') ||
    !kupRobots.includes('follow') ||
    kupOgTitle !== expectedKupTitle ||
    kupOgDesc !== expectedKupDesc ||
    kupOgUrl !== expectedKupCanonical ||
    kupTwTitle !== expectedKupTitle ||
    kupTwDesc !== expectedKupDesc ||
    !kupJsonLdTypes.includes('BreadcrumbList') ||
    !kupJsonLdTypes.includes('Product') ||
    !kupJsonLdTypes.includes('FAQPage') ||
    Number(kupProductObj?.offers?.lowPrice) !== 8350 ||
    Number(kupProductObj?.offers?.highPrice) !== 17100 ||
    !hasNoKupAvailability ||
    !hasNoKupPriceValidUntil ||
    kupFaqQuestionsCount !== 8 ||
    foundKupProhibited.length > 0 ||
    !hasKupGroupDivider
  ) {
    totalPassed = -1;
  }
