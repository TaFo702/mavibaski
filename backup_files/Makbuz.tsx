...
const SOZLESME_BASKI_DATA = [
  {
    ...
    rows: [
      { label: "1 Asl+1 Su.", values: [1550, 1900, 2500, 2750, 3200, 3500, 4400, 5050, 8300] },
      { label: "1 Asl+2 Su.", values: [1750, 2350, 3000, 3400, 4050, 4550, 5550, 6800, 11800] },
      { label: "1 Asl+3 Su.", values: [1900, 2600, 3400, 4050, 4700, 5400, 7000, 8300, 14800] },
      { label: "2. Renk Farkı", values: [650, 750, 900, 950, 1000, 1000, 1200, 1500, 2200] }
    ]
  }
];
...
const SIPARIS_FISI_DATA = [
  {
    ...
    rows: [
      { label: "1 Asl+1 Su.", values: [1500, 1550, 1900, 2500, 2800, 3300, 4300, 5050] },
      { label: "1 Asl+2 Su.", values: [1550, 1750, 2350, 3000, 3400, 4050, 5300, 7050] },
      { label: "1 Asl+3 Su.", values: [1650, 1900, 2600, 3400, 4050, 4700, 7200, 9200] },
      { label: "2. Renk Farkı", values: [650, 650, 750, 900, 1000, 1100, 1300, 1700] }
    ]
  }
];
...
const PARA_MAKBUZU_DATA = [
  {
    ...
    rows: [
      { label: "1 Asl+1 Su.", values: [1700, 1750, 2100, 2700, 3500, 5250] },
      { label: "1 Asl+2 Su.", values: [1750, 1950, 2550, 3200, 4250, 7250] },
      { label: "1 Asl+3 Su.", values: [1850, 2100, 2800, 3600, 4900, 9400] },
      { label: "2. Renk Farkı", values: [450, 450, 550, 700, 900, 1500] }
    ]
  }
];
...
const ADISYON_DATA = [
  {
    ...
    headers: ["5 CİLT", "10 CİLT", "20 CİLT", "40 CİLT", "50 CİLT", "100 CİLT", "200 CİLT", "300 CİLT", "500 CİLT", "1000 CİLT"],
    rows: [
      { label: "Fiyat", values: [1450, 1500, 1550, 2000, 2100, 2950, 4400, 5650, 9300, 16300] },
      { label: "Renk Farkı", values: [650, 650, 650, 700, 700, 950, 1100, 1300, 2200, 2700] }
    ]
  }
];
...
const TEDIYE_MAKBUZU_DATA = [
  {
    ...
    headers: ["5 CİLT", "10 CİLT", "20 CİLT", "30 CİLT", "50 CİLT", "100 CİLT"],
    rows: [
      { label: "1 Asıl + 1 Suret", values: [1700, 1750, 2100, 2700, 3500, 5250] },
      { label: "1 Asıl + 2 Suret", values: [1750, 1950, 2550, 3200, 4250, 7250] },
      { label: "1 Asıl + 3 Suret", values: [1850, 2100, 2800, 3600, 4900, 9400] },
      { label: "2. Renk Farkı", values: [450, 450, 550, 700, 900, 1500] }
    ]
  }
];
...
export const MakbuzFormlarPage = () => {
  ...
  const pageTitleForSEO = currentDetails
    ? currentDetails.seoTitle
    : pathId && pathId !== 'makbuz-ve-formlar'
      ? `${filteredData[0]?.title || "Makbuz ve Form"} Baskı Fiyatları - Mavi Basım`
      : "Makbuz, Reçete ve Sipariş Fişi Fiyatları - Mavi Basım";

  const pageDescForSEO = currentDetails
    ...
    : pathId && pathId !== 'makbuz-ve-formlar'
      ...
      : "Otokopili kendinden karbonlu adisyon, reçete, tahsilat makbuzu, sözleşme ve sipariş fişi matbaa baskı fiyatları. Topkapı imalathanemizden kapınıza hızlı teslimat.";

  if (currentDetails) {
    const item = filteredData[0];
    const halfFaqLength = Math.ceil(currentDetails.faqList.length / 2);
    const leftFaqs = currentDetails.faqList.slice(0, halfFaqLength);
    const rightFaqs = currentDetails.faqList.slice(halfFaqLength);
    ...
    const productSchema = {
      ...
      "category": pathId === 'para-makbuzu' ? "Matbu Evrak / Para Makbuzu" : "Matbu Evrak / Koçan Baskı",
      "material": pathId === 'para-makbuzu' ? "1. Sınıf Otokopili (Kendinden Karbonlu) Kağıt" : "1. Sınıf Otokopili Kağıt",
      "color": pathId === 'para-makbuzu' ? "Asıl Beyaz, Suretler Pembe/Sarı" : "Asıl Beyaz, Suretler Renkli",
      "size": pathId === 'para-makbuzu' ? "14x20 cm" : "Standart Koçan Ebadı",
    ...
    const videoSchema = pathId === 'para-makbuzu' ? {
      "@context": "https://schema.org",
      "@type": "VideoObject",
      "name": "Para Makbuzu ve Otokopili Koçan Baskı & Cilt Üretim Süreci",
      "description": "Mavi Basım Topkapı tesisimizde CTP kalıp pozlama, Heidelberg ofset baskı, kırmızı otomatik numaratör vurumu, perforaj ve tel dikiş ciltleme imalat adımları.",
      "thumbnailUrl": `https://mavibasim.com${currentDetails.featureImage}`,
      "uploadDate": "2026-01-15T09:00:00+03:00",
      "duration": "PT45S",
      "contentUrl": "https://mavibasim.com/video/para-makbuzu-uretim.mp4",
      "embedUrl": "https://mavibasim.com/video/para-makbuzu-uretim",
      "publisher": {
        "@type": "Organization",
        "name": "Mavi Basım",
        "logo": {
      ...
      <div className="bg-white min-h-screen pb-20">
        ...
        <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 pt-4 text-black">
            ...
            {pathId === 'siparis-fisi' || pathId === 'para-makbuzu' ? (
              <div className="text-slate-650 max-w-[1200px] mx-auto text-sm md:text-base text-left font-semibold space-y-4 mb-8 leading-relaxed">
                ...
                <div className="pt-4 flex flex-wrap items-center justify-center gap-4">
                  <a 
                    href="#fiyat-tablosu" 
                    className="bg-secondary hover:bg-primary text-white font-black text-xs md:text-sm px-6 py-3 rounded-full shadow-md transition-all uppercase tracking-tight flex items-center gap-2"
                  >
          ...
          <div className="bg-slate-50 border border-slate-200/90 rounded-2xl p-4 md:p-5 mb-8 text-left shadow-2xs">
            ...
            <nav className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 text-xs font-bold text-slate-700" aria-label="Sayfa İçindekiler Navigasyonu">
              <a href="#fiyat-tablosu" className="bg-white hover:bg-primary hover:text-white border border-slate-200 p-2.5 rounded-xl transition-all flex items-center gap-1.5 shadow-2xs group">
                <span className="text-primary group-hover:text-white font-black">1.</span> Fiyat Tablosu
              </a>
              <a href="#ozellikler" className="bg-white hover:bg-primary hover:text-white border border-slate-200 p-2.5 rounded-xl transition-all flex items-center gap-1.5 shadow-2xs group">
                <span className="text-primary group-hover:text-white font-black">2.</span> Özellikler
              </a>
              <a href="#kimler-kullanir" className="bg-white hover:bg-primary hover:text-white border border-slate-200 p-2.5 rounded-xl transition-all flex items-center gap-1.5 shadow-2xs group">
                <span className="text-primary group-hover:text-white font-black">3.</span> Kimler Kullanır
              </a>
              <a href="#uretim-sureci" className="bg-white hover:bg-primary hover:text-white border border-slate-200 p-2.5 rounded-xl transition-all flex items-center gap-1.5 shadow-2xs group">
                <span className="text-primary group-hover:text-white font-black">4.</span> Üretim Süreci
              </a>
              <a href="#nasil-doldurulur" className="bg-white hover:bg-primary hover:text-white border border-slate-200 p-2.5 rounded-xl transition-all flex items-center gap-1.5 shadow-2xs group">
                <span className="text-primary group-hover:text-white font-black">5.</span> Nasıl Doldurulur
              </a>
              <a href="#sss-bolumu" className="bg-white hover:bg-primary hover:text-white border border-slate-200 p-2.5 rounded-xl transition-all flex items-center gap-1.5 shadow-2xs group">
                <span className="text-primary group-hover:text-white font-black">6.</span> Sıkça Sorulan Sorular
              </a>
            </nav>
          </div>
          ...
          {/* TEKNİK ÖZELLİKLER */}
          <div className={pathId === 'gider-makbuzu' || pathId === 'tahsilat-makbuzu' ? 'hidden' : 'mb-12'}>
            <div className="flex justify-between items-center mb-6 px-2">
              <div className="flex items-center gap-3">
                <div className="w-2 h-8 bg-primary rounded-full" />
                <h2 className="text-xl md:text-2xl font-black text-black uppercase tracking-tight">
                  ...
                  <>
                    <div className="border-t border-gray-100 pt-8">
                      <h2 className="text-xl md:text-2xl font-black text-black mb-4 uppercase tracking-tight flex items-center gap-2">
                        Numaratörlü Adisyon Siparişi Nasıl Verilir?
                      </h2>
          ...
          <div className="bg-white border border-gray-150 rounded-3xl p-8 md:p-12 shadow-sm mb-14 relative overflow-hidden space-y-12 text-black">
            ...
            {(pathId === 'gider-makbuzu' || pathId === 'tahsilat-makbuzu') && (
              <div className="relative z-10 border-t border-gray-100 pt-8">
                <h2 className="text-xl md:text-2xl font-black text-black mb-4 uppercase tracking-tight flex items-center gap-2">
                  <span className="w-1.5 h-6 bg-secondary rounded-full" />
                  {pathId === 'gider-makbuzu' ? 'Özel Tasarım Gider Makbuzu Baskı' : 'Özel Tasarım Tahsilat Makbuzu Baskı'}
                </h2>
                <div className="text-slate-650 text-base leading-relaxed font-semibold space-y-4 text-left md:text-justify text-black">
                  {pathId === 'gider-makbuzu' ? (
                    <>
            ...
            {(pathId === 'gider-makbuzu' || pathId === 'tahsilat-makbuzu') && (
              <div className="relative z-10 border-t border-gray-100 pt-8">
                <h2 className="text-xl md:text-2xl font-black text-black mb-4 uppercase tracking-tight flex items-center gap-2">
                  <span className="w-1.5 h-6 bg-secondary rounded-full" />
                  {pathId === 'gider-makbuzu' ? 'Gider Makbuzu Siparişi Nasıl Verilir?' : 'Tahsilat Makbuzu Siparişi Nasıl Verilir?'}
                </h2>
                <div className="text-slate-650 text-base leading-relaxed font-semibold space-y-4 text-left md:text-justify text-black">
                  <p>
                    {pathId === 'gider-makbuzu' ? (
            ...
            {pathId !== 'arac-kiralama' ? (
              <div className={`relative z-10 grid grid-cols-1 ${pathId === 'gider-makbuzu' ? '' : 'md:grid-cols-2'} gap-8 border-t border-gray-100 pt-8`}>
                {/* Nasıl Doldurulur? */}
            ...
            {pathId === 'para-makbuzu' && (
              <>
                ...
                <div className="relative z-10 border-t border-gray-100 pt-8">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {[
                      { title: "Tahsilat Makbuzu", path: "/makbuz/tahsilat-makbuzu", desc: "Müşterilerden alınan ödeme ve alacakların resmi kayıt altına alınması için." },
                      { title: "Tediye Makbuzu", path: "/makbuz/tediye-makbuzu", desc: "Kasadan tedarikçi ve üçüncü şahıslara yapılan nakit çıkışlarının belgelenmesi için." },
                      { title: "Gider Makbuzu", path: "/makbuz/gider-makbuzu", desc: "Şirket içi harcama ve küçük masrafların düzenli kaydı için." },
                      { title: "Sipariş Fişi", path: "/makbuz/siparis-fisi", desc: "Alınan ürün ve hizmet siparişlerinin detaylı takibi için." },
                      { title: "Adisyon Baskı", path: "/makbuz/adisyon", desc: "Hizmet ve gıda sektörüne özel masasal adisyon takibi için." },
                      { title: "Servis Formu", path: "/makbuz/arac-kiralama", desc: "Teknik servis ve teslimat süreçlerinin kayıt altına alınması için." }
                    ].map((linkItem, idx) => (
            ...
            {pathId === 'para-makbuzu' && (
              <>
                {/* 1. Para Makbuzu Düzenleme Esasları ve Hukuki Boyutu */}
                <div className="relative z-10 border-t border-gray-100 pt-8">
                  <h2 className="text-xl md:text-2xl font-black text-black mb-4 uppercase tracking-tight flex items-center gap-2">
                    Para Makbuzu Düzenleme Esasları ve Hukuki Boyutu
                  </h2>
                  <div className="text-slate-650 text-sm md:text-base leading-relaxed font-medium space-y-4">
                    <p>
                      Günlük ticari faaliyetlerde ve kişisel işlemlerde, elden gerçekleştirilen nakit transferlerinin güvenli ve şeffaf bir şekilde belgelenmesi önemlidir. Kurumsal yapıda hazırlanan bir <strong>para makbuzu</strong>, taraflar arasında yazılı kayıt oluşturulmasına ve uyuşmazlık durumlarında destekleyici belge olarak kullanılmasına yardımcı olur.
                    </p>
                    <p>
                      Özel veya ticari amaçla tasarlanan bu evrak, parayı teslim alan taraf ile teslim eden taraf arasında karşılıklı mutabakat ve kayıt düzeni oluşturur.
                    </p>
                  </div>
                </div>

                {/* 2. Para Makbuzunda Tahrifatı Önleme ve Doğru Doldurma Yöntemleri */}
                <div className="relative z-10 border-t border-gray-100 pt-8">
                  <h2 className="text-xl md:text-2xl font-black text-black mb-4 uppercase tracking-tight flex items-center gap-2">
                    Para Makbuzunda Tahrifatı Önleme ve Doğru Doldurma Yöntemleri
                  </h2>
                  <div className="text-slate-650 text-sm md:text-base leading-relaxed font-medium space-y-4">
                    <p>
                      Para makbuzunun doldurulma aşamasında unvanların ve şahıs isimlerinin kimlik ve resmi kayıtlardaki haliyle birebir uyuşması büyük önem taşır. Belgenin güvenliğini artırmak ve sonradan müdahale riskini önlemek için şu hususlara dikkat edilmelidir:
                    </p>
                    <ul className="list-disc pl-5 space-y-2">
                      <li>
                        <strong>Tutar Koruması:</strong> Rakamla yazılan tutarın sonuna kuruş hanesi virgülle eklenmeli ve başına-sonuna kapatma işareti koyulmalıdır (örneğin; `#2.500,00 TL#`).
                      </li>
                      <li>
                        <strong>Yazıyla Tutar Beyanı:</strong> Tutarın yazıyla ifade edildiği bölüm `Yalnız İki Bin Beş Yüz Türk Lirası` şeklinde boşluk bırakılmadan kapatılarak yazılmalıdır.
                      </li>
                      <li>
                        <strong>Suret Muhafazası:</strong> Otokopili (kendinden karbonlu) koçanda üst asıl nüsha ödeme yapan tarafa teslim edilirken, alt suret koçan bünyesinde arşivlenir.
                      </li>
                    </ul>
                  </div>
                </div>

                {/* 3. Para Makbuzu Resmi Evrak Mıdır? */}
                <div className="relative z-10 border-t border-gray-100 pt-8">
                  <h2 className="text-xl md:text-2xl font-black text-black mb-4 uppercase tracking-tight flex items-center gap-2">
                    Para Makbuzu Resmi Evrak Mıdır?
                  </h2>
                  <div className="text-slate-650 text-sm md:text-base leading-relaxed font-medium space-y-4">
                    <div className="bg-primary/[0.03] border-l-4 border-primary p-5 rounded-r-2xl my-2 text-slate-800 text-sm md:text-base leading-relaxed shadow-sm">
                      <p className="font-bold text-black mb-1">📋 Özet Bilgilendirme:</p>
                      <p>
                        Para makbuzu Vergi Usul Kanunu kapsamında fatura yerine geçen resmi mali bir vergi belgesi değildir. Ancak taraflar arasında elden yapılan ödeme veya para teslimlerinin belgelendirilmesini sağlayan iç ticari evraktır.
                      </p>
                    </div>
                    <p>
                      Ticari işletmeler ve şahıslar arası işlemlerde fatura veya e-arşiv fatura düzenlenmesi esas olmakla birlikte; elden yapılan kapora, depozito, şantiye elden masraf çıkışları ve geçici para teslimlerinde para makbuzu kayıt düzeni oluşturur.
                    </p>
                  </div>
                </div>

                {/* 4. Fiziki Nakit Hareketlerinde Belge Düzeni Sağlayan Senaryolar */}
                <div className="relative z-10 border-t border-gray-100 pt-8">
                  <h2 className="text-xl md:text-2xl font-black text-black mb-4 uppercase tracking-tight flex items-center gap-2">
                    Fiziki Nakit Hareketlerinde Belge Düzeni Sağlayan Senaryolar
                  </h2>
                  <div className="text-slate-650 text-sm md:text-base leading-relaxed font-medium space-y-4">
                    <p>
                      Nakit transferinin gerçekleştiği anlarda belirsizliklerin ve uyuşmazlıkların önüne geçmek için para makbuzu kullanımı tercih edilmektedir:
                    </p>
                    <ul className="list-disc pl-5 space-y-2">
                      <li>
                        <strong>Gayrimenkul Kaporaları:</strong> Kiralama veya satın alma öncesi verilen ön ödemelerde elden teslim kaydı oluşturur.
                      </li>
                      <li>
                        <strong>Şantiye ve Saha Masrafları:</strong> Taşeron veya usta elden avans ve hak ediş ödemelerinde düzenli kayıt sağlar.
                      </li>
                      <li>
                        <strong>Depozito ve Teminat İşlemleri:</strong> Ofis, araç veya ekipman kiralamalarında alınan depozitoların iade süreçlerinde referans oluşturur.
                      </li>
                      <li>
                        <strong>Dernek ve Vakıf İşlemleri:</strong> Makbuz karşılığı toplanan aidat veya bağış ödemelerinde yazılı kayıt sunar.
                      </li>
                    </ul>
                  </div>
                </div>

                {/* 5. Para Makbuzunda Bulunması Gereken Bilgi Alanları */}
                <div className="relative z-10 border-t border-gray-100 pt-8">
                  <h2 className="text-xl md:text-2xl font-black text-black mb-4 uppercase tracking-tight flex items-center gap-2">
                    Para Makbuzunda Bulunması Gereken Bilgi Alanları
                  </h2>
                  <div className="text-slate-650 text-sm md:text-base leading-relaxed font-medium space-y-4">
                    <p>
                      Standart ve işlevsel bir para makbuzu koçanında aşağıdaki temel bilgi alanları yer almalıdır:
                    </p>
                    <ul className="list-disc pl-5 space-y-2.5">
                      <li><strong>Tarih Alanı:</strong> Paranın teslim edildiği gün, ay ve yıl.</li>
                      <li><strong>Seri Numarası (Numaratör):</strong> Ardışık sıra takibi sağlayan 6 haneli kırmızı numaratör baskısı.</li>
                      <li><strong>Ödeme Yapan Bilgileri:</strong> Ödemeyi gerçekleştiren şahıs veya firma adı.</li>
                      <li><strong>Ödeme Alan Bilgileri:</strong> Parayı teslim alan şahıs veya yetkili bilgisi.</li>
                      <li><strong>Tutar (Rakam ve Yazı ile):</strong> Hatalı yorumlamayı önleyen çift tutar beyanı.</li>
                      <li><strong>Açıklama:</strong> Paranın hangi gerekçeyle teslim edildiğini belirten detay alanı.</li>
                      <li><strong>Islak İmzalar:</strong> Her iki tarafın onayını gösteren imza bölümleri.</li>
                    </ul>
                  </div>
                </div>

                {/* 6. Para Makbuzu, Tahsilat Makbuzu ve Tediye Makbuzu Karşılaştırması */}
                <div className="relative z-10 border-t border-gray-100 pt-8">
                  <h2 className="text-xl md:text-2xl font-black text-black mb-4 uppercase tracking-tight flex items-center gap-2">
                    Para Makbuzu, Tahsilat Makbuzu ve Tediye Makbuzu Karşılaştırması
                  </h2>
                  <div className="text-slate-650 text-sm md:text-base leading-relaxed font-medium space-y-4">
                    <p>
                      İşletmelerin operasyonlarında sıklıkla karıştırılan matbu evrak türleri ve kullanım amaçları aşağıdaki gibidir:
                    </p>
                    <div className="max-w-xl overflow-x-auto my-4 border border-gray-150 rounded-2xl shadow-2xs">
                      <table className="w-full text-left border-collapse">
                        <thead>
                          <tr className="bg-slate-100 border-b border-slate-200">
                            <th className="p-3 font-black text-xs md:text-sm text-black">Evrak Türü</th>
                            <th className="p-3 font-black text-xs md:text-sm text-black">Temel Kullanım Amacı</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-100">
                          <tr>
                            <td className="p-3 font-bold text-xs md:text-sm text-slate-900">Para Makbuzu</td>
                            <td className="p-3 text-xs md:text-sm text-slate-700">Elden yapılan genel para teslimleri, kapora ve geçici nakit hareketlerinin kaydı.</td>
                          </tr>
                          <tr className="bg-slate-50/50">
                            <td className="p-3 font-bold text-xs md:text-sm text-slate-900"><Link to="/tahsilat-makbuzu" className="text-primary hover:underline font-bold">Tahsilat Makbuzu</Link></td>
                            <td className="p-3 text-xs md:text-sm text-slate-700">Müşterilerden mal veya hizmet karşılığı yapılan cari hesap tahsilatları.</td>
                          </tr>
                          <tr>
                            <td className="p-3 font-bold text-xs md:text-sm text-slate-900"><Link to="/tediye-makbuzu" className="text-primary hover:underline font-bold">Tediye Makbuzu</Link></td>
                            <td className="p-3 text-xs md:text-sm text-slate-700">Şirket kasasından tedarikçi veya gider ödemeleri için yapılan resmi nakit çıkışları.</td>
                          </tr>
                        </tbody>
                      </table>
                    </div>
                ...
                {/* 7. Numaratörlü ve Otokopili Makbuz Kullanmanın İşletmeye Faydaları */}
                <div className="relative z-10 border-t border-gray-100 pt-8">
                  <h2 className="text-xl md:text-2xl font-black text-black mb-4 uppercase tracking-tight flex items-center gap-2">
                    Numaratörlü ve Otokopili Makbuz Kullanmanın İşletmeye Faydaları
                  </h2>
                  <div className="text-slate-650 text-sm md:text-base leading-relaxed font-medium space-y-4">
                    <p>
                      Ardışık kırmızı numaratör baskısı, işlemlerin sıra numarasına göre takip edilmesini kolaylaştırır ve kayıt düzeni sağlar. Kendinden karbonlu otokopi kağıdı ise araya karbon kağıdı koyma zahmetini ortadan kaldırarak ikinci nüshaya yazının pürüzsüz aktarılmasını garantiler.
                    </p>
                  </div>
                </div>

                {/* 8. Para Makbuzu Kullanırken Yapılan Yaygın Hatalar */}
                <div className="relative z-10 border-t border-gray-100 pt-8 shadow-none">
                  <h2 className="text-xl md:text-2xl font-black text-black mb-4 uppercase tracking-tight flex items-center gap-2">
                    Para Makbuzu Kullanırken Yapılan Yaygın Hatalar
                  </h2>
                  <div className="text-slate-650 text-xs md:text-sm leading-relaxed font-semibold space-y-4">
                    <p>
                      Para makbuzu doldurulurken yapılan en büyük hatalardan biri, tarih bilgisinin boş bırakılması veya parayı teslim eden tarafın imzasının alınmamasıdır. Ayrıca tutarın rakamla ve yazıyla olan hanelerinin uyuşmaması, yasal uyuşmazlıklarda belgenin geçerliliğini zayıflatabilir. Mavi Basım olarak hazırladığımız şablonlar, bu hataları önleyecek şekilde belirgin alanlara sahiptir.
                    </p>
                  </div>
                </div>
              </>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-3.5">
                {leftFaqs.map((faq, idx) => {
                  ...
                  const isOpen = isForcedOpen || activeFaq === fIdx;
                  return (
                    <div key={fIdx} className="border border-gray-150 rounded-2xl overflow-hidden bg-gray-50/50 hover:bg-white transition-all text-black">
                      <button 
                        onClick={() => !isForcedOpen && setActiveFaq(activeFaq === fIdx ? null : fIdx)}
                        disabled={isForcedOpen}
                        className={`w-full text-left p-5 flex justify-between items-center font-bold text-slate-900 focus:outline-none transition-all ${isForcedOpen ? "cursor-default text-emerald-950" : ""}`}
                      >
                        <h3 className="md:text-base text-sm leading-relaxed pr-4 text-black font-bold">{faq.q}</h3>
                        {!isForcedOpen && (
                          <ChevronDown 
                            size={20} 
                            className={`text-primary transform transition-transform duration-300 shrink-0 ${activeFaq === fIdx ? "rotate-180" : ""}`} 
                          />
                        )}
                      </button>
                      <motion.div 
                        initial={isForcedOpen ? { height: "auto", opacity: 1 } : false}
                        animate={{ 
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3 shrink-0">
                <div className="bg-slate-800/80 p-3 rounded-2xl border border-slate-700/60 text-center">
                  ...
                  <span className="text-xs font-bold text-white">2004'ten Beri Üretim</span>
                </div>
                <div className="bg-slate-800/80 p-3 rounded-2xl border border-slate-700/60 text-center">
                  <span className="block text-emerald-400 font-black text-sm mb-0.5">✓</span>
    ...
    <div className="bg-white min-h-screen pb-20">
      ...
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        <div className="text-center mb-12">
          <h1 className="text-2xl md:text-3xl font-black text-primary uppercase tracking-tighter mb-4">
            {pageTitle} Baskı ve Tasarım Çözümleri
          </h1>
            ...
            <div key={idx} id={item.id} className="scroll-mt-24 group">
              <div className="flex justify-between items-center mb-6 px-2">
                <div className="flex items-center gap-3">
                  <div className="w-2 h-8 bg-secondary rounded-full" />
                  <h2 className="text-xl md:text-2xl font-black text-black uppercase tracking-tight">
                    {item ? item.title : "Ürün"} - {item ? item.ebat : "Ebat"} Fiyat Listesi
                  </h2>
                </div>
              </div>
              {item && <SiparisFisiHorizontalTable data={[item]} />}
